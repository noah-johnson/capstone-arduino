...
//Returns an array of Strings for all the records
//
//
//
//
//
//
String[] NdefMessage::getString()
{
	recordString[_recordCount];
	int i;
	for (i = 0; i < _recordCount; i++)
	{
		recordString[i] = _records[i].getString();
		Serial.println("Record is " + recordString[i]);
	}
	return recordString;
}