...
//
//
String NfcTag::getData()
{
	Serial.println("returning data as an array of strings");
	if (_ndefMessage == NULL)
	{
		Serial.println(F("\nNo NDEF Message"));
		//TODO; deal with this case -- maybe with try and catch in method call
	}
	else
	{
		
/* 		vector<string> data = _ndefMessage->getString();
        std::string finalMessage = "";
		for (int i =0; i < data.size(); i++) {
		    finalMessage.append(data[i]);
		}
 */
		return _ndefMessage->getString();
	}
}